module.exports = function(App, Firefly) {
	...
	var Filesystem = new Class({
		...
		path: function(substr) {
			if(substr === undefined) {
				substr = '';
			}
			return this._path.substr(substr.length);
		},
		join: function(first) {
			var args = Object.values(arguments);
			if(typeof first == 'object') {
		...
		cd: function() {
			this._path = this.join.apply(this, Object.values(arguments));
			return true;
		},
		find: function(callback) {
				...
				//i love this way of doing recursive things!!! node is kick ass!
				var recursiveHelper = function(values) {
					values.each(function(filesystemObject) {
							//check if our given filesystem object is a file
							if(filesystemObject.isFile()) {
								results.push(filesystemObject);
							} else {
								recursiveHelper(filesystemObject.readdir());
							}
						});
				}
				
				recursiveHelper(this.readdir());
				
				callback(null, results);
		},
		basename: function(substr) {
			var basename = path.basename(this._path);
			if(typeof substr !== 'undefined') {
				basename = basename.substr(substr.length);
			}
			return basename;
		},
		...
		isFile: function(callback) {
			...
			return this.stat(function(err, stat) {
		...
		isBlockDevice: function(callback) {
			...
			return this.stat(function(err, stat) {
				callback(err, stat.isBlockDevice());
			});
		},
		isCharacterDevice: function(callback) {
			...
			return this.stat(function(err, stat) {
				callback(err, stat.isCharacterDevice());
			});
		},
		isSymbolicLink: function(callback) {
			...
			return this.lstat(err, function(lstat) {
				callback(err, lstat.isSymbolicLink());
			});
		},
