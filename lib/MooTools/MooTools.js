...
Object.each(Inflection, function(value, index) {
	if(typeof value == 'function') {
		var tmp = {};
		if(typeof String.prototype[index] !== 'undefined') {
			switch(index) {
				case 'split':
					index = 'camelSplit';
					break;
				default:
					return; /* we should not override metodes already defined */
			}
		}
		tmp[index] = function() {
...
Array.implement({
	compare: function(testArr) {
		...
		for (var i = 0; i < testArr.length; i++) {
			if (
				(this[i].compare && !this[i].compare(testArr[i])) ||
				(this[i] !== testArr[i])
			) { 
				return false;
			}
...
String.implement({
	...
	rtrim: function(charlist) {
		//TODO: Ugly code! do not follow DRY!
